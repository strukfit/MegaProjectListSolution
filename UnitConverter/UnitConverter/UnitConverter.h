...
public:
    UnitConverter(QWidget *parent = nullptr);
    ~UnitConverter();
...
private:
    void fillUnitsComboBoxes(int index);
    void convert_value(QComboBox* comboFrom, QComboBox* comboTo, QLineEdit* lineFrom, QLineEdit* lineTo);
    double get_conversion_factor(QString name, QString unitType);

    Ui::UnitConverterClass* ui;
    ...
    std::map<QString, std::vector<MeasurementUnit>> unitsMap = {
    ...
    std::map<QString, std::map<QString, TemperatureConversion>> temperatureConversions = {
        {"Celsius", {
            {"Fahrenheit", {1.8, 32.0}},
            {"Kelvin", {1.0, 273.15}}
        }},
